...
export default class NewItem1 extends React.Component{
    ...
    fatchData = async () => {
        try{
            ...
            const sub_id = await AsyncStorage.getItem('sub_id');   
            console.log("Cache Data : " + sub_id);
            const s_id = await AsyncStorage.getItem('shop_id');
            console.log(s_id);

            let sql = "SELECT p_list_id, p_name, m1.manu_name, pic_1 FROM product_list_table As P "+
                "INNER JOIN manufacture_list_table As m1 ON m1.manu_id = P.manufacture_id "+ 
                "Where sub_category_id = '"+sub_id+"' AND p_list_id NOT IN ("+
                " SELECT DISTINCT p1.p_list_id FROM product_list_table As p1 "+ 
                "LEFT JOIN product_table As p2 ON p1.p_list_id = p2.p_list_id "+ 
                "WHERE p1.sub_category_id ='"+sub_id+"' And p2.shop_id = '"+s_id+"')";
            ...
            body: JSON.stringify({
                ...
                .then((responseJson) => {
                        ...
                        //console.log("Data Seted Complited.");                  
                }).catch((error) => {
                    console.error(error);
            });
        }
